...
export function explode(pos, power = 1, { nature = false, fire = true } = {}) {
  ...
  /* a real fireball sets the nearest tree alight more often than not */
  if (fire && !nature) igniteNear(pos, 6 + p * 4, 0.45 + p * 0.2);
}
...
export function burnTick(e, dt) {
  ...
  if (f < 0.3) {
    ...
    /* a burning building lights the treeline beside it */
    if (e.team !== 'wild' && Math.random() < 0.05) igniteNear(e.pos, e.def.radius + 7, 1);
  }
}

/* ------------------------------------------------------- burning trees --- */
/* A firefight leaves marks on the forest. Explosions, turret shell bursts,
   burning structures and (rarely) a stray tracer can set the nearest tree
   alight; a burning tree blazes for a while, throws embers and smoke, lights
   its surroundings from the pooled PointLights, occasionally passes the fire
   to a neighbour, and then stands there CHARRED for the rest of the match.

   Capped hard. Seven fires at once is a battle; seventy is a map on fire, and
   the whole point of concealment (canopy fade, fog) dies with the canopy.
   Rain and snow damp the burn -- see rainfall() -- so a storm map self-limits. */
const MAX_FIRES = 7;
const fires = [];
const charredBy = new WeakMap();          // leaves mesh -> Set(instance index)
const _fm4 = new THREE.Matrix4(), _fp = new THREE.Vector3(),
      _fq = new THREE.Quaternion(), _fs = new THREE.Vector3();
const CHAR_LEAF = new THREE.Color(0x1f1610);
const CHAR_BARK = new THREE.Color(0.42, 0.36, 0.32);

function treeAlreadyBurning(leaves, idx) {
  for (const f of fires) if (f.leaves === leaves && f.idx === idx) return true;
  return false;
}

/* Set the nearest un-burnt tree within `radius` of pos alight, with the given
   probability. Returns true if something caught. Pure VFX: no gameplay reads
   this, so it can never decide a fight. */
export function igniteNear(pos, radius = 8, chance = 1) {
  if (!G.canopies || !G.canopies.length) return false;
  if (fires.length >= MAX_FIRES) return false;
  if (Math.random() > chance) return false;
  let best = null, bd = radius * radius;
  for (const leaves of G.canopies) {
    const P = leaves.userData.pos; if (!P) continue;
    const burnt = charredBy.get(leaves);
    const act = leaves.userData.active;
    const nn = act ? act.length : leaves.count;
    for (let k = 0; k < nn; k++) {
      const i = act ? act[k] : k;
      if (burnt && burnt.has(i)) continue;
      const dx = P[i * 3] - pos.x, dz = P[i * 3 + 2] - pos.z;
      const d2 = dx * dx + dz * dz;
      if (d2 < bd) { bd = d2; best = { leaves, i }; }
    }
  }
  if (!best || treeAlreadyBurning(best.leaves, best.i)) return false;
  startFire(best.leaves, best.i);
  return true;
}

function startFire(leaves, idx) {
  const P = leaves.userData.pos;
  leaves.getMatrixAt(idx, _fm4);
  _fm4.decompose(_fp, _fq, _fs);
  const canopyY = (leaves.userData.canopyY || 6.7) * _fs.y;
  fires.push({
    leaves, idx,
...
function finishFire(f) {
  let set = charredBy.get(f.leaves);
  if (!set) charredBy.set(f.leaves, set = new Set());
  set.add(f.idx);
  f.leaves.setColorAt(f.idx, CHAR_LEAF);
  f.leaves.instanceColor.needsUpdate = true;
  const twin = f.leaves.userData.twin;
...
function updateFires(dt) {
  ...
  /* weather damps the burn: a downpour puts a tree out in a third of the time */
  const wet = typeof rainfall === 'function' ? rainfall() : 0;
  const decay = 1 + wet * 2.2;
  const G_ = G;
  for (let i = fires.length - 1; i >= 0; i--) {
    const f = fires[i];
    f.t += dt * decay;
    const k = f.t / f.life;                       // 0 → 1
    if (k >= 1) { finishFire(f); fires.splice(i, 1); continue; }
    /* intensity: catches, blazes, gutters */
    const heat = k < 0.15 ? k / 0.15 : k > 0.75 ? (1 - k) / 0.25 : 1;

    /* canopy tint: green -> ember-orange -> brown as it burns through */
    _c.copy(f.c0);
    if (k < 0.5) _c.lerp(FIRE_B, k * 1.6);
    else _c.lerpColors(FIRE_B, CHAR_LEAF, (k - 0.5) * 2);
    f.leaves.setColorAt(f.idx, _c);
    f.leaves.instanceColor.needsUpdate = true;
  ...
  G_.fireCount = fires.length;
}

export function resetFires() { fires.length = 0; }
